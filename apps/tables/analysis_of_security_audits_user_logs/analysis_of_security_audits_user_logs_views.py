...
from apps.tables.models import (
    ...
    FloatRangeFilter, 
    ModelChoices, 
    AnalysisofSecurityAuditsUserLogs,
    Tab, SelectedRows
)
...
def create_analysis_of_security_audits_user_logs_filter(request):
    if request.method == 'POST':
        user_filter_common_func(request, 'AnalysisofSecurityAuditsUserLogs', 'ANALYSISOF_SECURITY_AUDITS_USER_LOGS')
    return redirect(request.META.get('HTTP_REFERER'))

def create_analysis_of_security_audits_user_logs_page_items(request):
    if request.method == 'POST':
        create_page_items(request, 'ANALYSISOF_SECURITY_AUDITS_USER_LOGS')
        return redirect(request.META.get('HTTP_REFERER'))

def create_analysis_of_security_audits_user_logs_hide_show_filter(request):
    if request.method == "POST":
        response_data = create_hide_show_filter(request, 'ANALYSISOF_SECURITY_AUDITS_USER_LOGS')
        return JsonResponse(response_data)

    return JsonResponse({'error': 'Invalid request'}, status=400)
    
def delete_analysis_of_security_audits_user_logs_filter(request, id):
    filter_instance = UserFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.ANALYSISOF_SECURITY_AUDITS_USER_LOGS)
    filter_instance.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_analysis_of_security_audits_user_logs_daterange_filter(request, id):
    date_filter = DateRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.ANALYSISOF_SECURITY_AUDITS_USER_LOGS)
    date_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_analysis_of_security_audits_user_logs_intrange_filter(request, id):
    int_filter = IntRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.ANALYSISOF_SECURITY_AUDITS_USER_LOGS)
    int_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_analysis_of_security_audits_user_logs_floatrange_filter(request, id):
    float_filter = FloatRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.ANALYSISOF_SECURITY_AUDITS_USER_LOGS)
    float_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def analysis_of_security_audits_user_logs(request):
    db_field_names = [field.name for field in AnalysisofSecurityAuditsUserLogs._meta.get_fields() if not field.is_relation]
    try:
        user_order = ColumnOrder.objects.get(
            user=request.user, 
            table_name='AnalysisofSecurityAuditsUserLogs',
    ...
    selected_rows = SelectedRows.objects.filter(
        model='analysisofsecurityauditsuserlogs', 
        model_choice=ModelChoices.ANALYSISOF_SECURITY_AUDITS_USER_LOGS
    ).values_list('rows', flat=True)
    selected_rows = [int(item) for row in selected_rows for item in row.split(',')]

    # Snapshot
    latest_snapshot, summary, snapshot_filter, snapshots = common_snapshot_filter(request, 'analysisofsecurityauditsuserlogs')

    field_names = []
    for field_name in ordered_fields:
        fields, created = HideShowFilter.objects.get_or_create(key=field_name, userID=get_user_id(request), parent=ModelChoices.ANALYSISOF_SECURITY_AUDITS_USER_LOGS)
        field_names.append(fields)

    page_items = PageItems.objects.filter(userID=get_user_id(request), parent=ModelChoices.ANALYSISOF_SECURITY_AUDITS_USER_LOGS).last()
    items = 25
    if page_items:
    ...
    filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.ANALYSISOF_SECURITY_AUDITS_USER_LOGS, field_type=FieldType.TEXT)
    ...
    # for date range
    date_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.ANALYSISOF_SECURITY_AUDITS_USER_LOGS, field_type=FieldType.DATE)
    ...
    # for integer range
    int_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.ANALYSISOF_SECURITY_AUDITS_USER_LOGS, field_type=FieldType.INT)
    ...
    # for float range
    float_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.ANALYSISOF_SECURITY_AUDITS_USER_LOGS, field_type=FieldType.FLOAT)
    ...
    base_queryset = AnalysisofSecurityAuditsUserLogs.objects.filter(**filter_string).filter(**pre_filter_string).filter(combined_q_objects).filter(**snapshot_filter)
    ...
    if user_unique_filter:
        queryset = common_unique_filter(request, user_unique_filter, queryset, snapshot_filter, 'tables_analysisofsecurityauditsuserlogs')
    if date_unique_filter:
        queryset = common_unique_filter(request, date_unique_filter, queryset, snapshot_filter, 'tables_analysisofsecurityauditsuserlogs')
    if int_unique_filter:
        queryset = common_unique_filter(request, int_unique_filter, queryset, snapshot_filter, 'tables_analysisofsecurityauditsuserlogs')
    if float_unique_filter:
        queryset = common_unique_filter(request, float_unique_filter, queryset, snapshot_filter, 'tables_analysisofsecurityauditsuserlogs')
    ...
    try:
        ...
        return redirect('analysis_of_security_audits_user_logs')
    except EmptyPage:
        return redirect('analysis_of_security_audits_user_logs') 

    read_only_fields = ('ID', 'hash_data', 'loader_instance', 'json_data',)
    pre_column = ('ID', 'hash_data', 'loader_instance', 'json_data',)
    compulsory_fields = ()
    
    COMBOS = {}
    #COMBOS['Contract_Type'] = TableDropdownSubItem.objects.filter(item__item='Contract_Type').values_list('subitem', flat=True)

    fields = get_model_fields('AnalysisofSecurityAuditsUserLogs', pre_column)
    saved_filters = list(SavedFilter.objects.filter(
        userID=request.user.id, parent=ModelChoices.ANALYSISOF_SECURITY_AUDITS_USER_LOGS
    ).values())
    ...
    context = {
        'segment'  : 'analysis_of_security_audits_user_logs',
        'parent'   : 'uam',
        'softwares' : softwares,
        ...
        'total_items': AnalysisofSecurityAuditsUserLogs.objects.filter(**pre_filter_string).filter(**snapshot_filter).count(),
        'pre_column': pre_column,
        'COMBOS': COMBOS,
        'date_picker_fields': AnalysisofSecurityAuditsUserLogs.date_fields_to_convert,
        'compulsory_fields': compulsory_fields,
        'fav_name': 'Analysis of Security Audits User Logs',
        'int_filter_instance': int_filter_instance,
        'int_fields': AnalysisofSecurityAuditsUserLogs.integer_fields,
        'float_filter_instance': float_filter_instance,
        'float_fields': AnalysisofSecurityAuditsUserLogs.float_fields,
        'encrypted_fields': AnalysisofSecurityAuditsUserLogs.encrypted_fields,
        ...
        'tabs': Tab.objects.filter(base_view='analysis_of_security_audits_user_logs').order_by('created_at'),
        'join_model_instance': AnalysisofSecurityAuditsUserLogs.join_model_instance if hasattr( AnalysisofSecurityAuditsUserLogs, 'join_model_instance') else None,
    ...
    return render(request, 'apps/analysis_of_security_audits_user_logs.html', context)


@login_required(login_url='/users/signin/')
def create_analysis_of_security_audits_user_logs(request):
    if request.method == 'POST':
        software_data = {}
        for attribute, value in request.POST.items():
            ...
            if attribute not in AnalysisofSecurityAuditsUserLogs.date_fields_to_convert:
                if attribute in AnalysisofSecurityAuditsUserLogs.integer_fields:
                    software_data[attribute] = int(value) if value else None
                elif attribute in AnalysisofSecurityAuditsUserLogs.float_fields:
        ...
        AnalysisofSecurityAuditsUserLogs.objects.create(**software_data)

    return redirect(request.META.get('HTTP_REFERER'))

@login_required(login_url='/users/signin/')
def delete_analysis_of_security_audits_user_logs(request, id):
    software = AnalysisofSecurityAuditsUserLogs.objects.get(ID=id)
    software.delete()
    return redirect(request.META.get('HTTP_REFERER'))


@login_required(login_url='/users/signin/')
def update_analysis_of_security_audits_user_logs(request, id):
    software = AnalysisofSecurityAuditsUserLogs.objects.get(ID=id)
    ...
    if request.method == 'POST':
        for attribute, value in request.POST.items():
            ...
            if not attribute in AnalysisofSecurityAuditsUserLogs.date_fields_to_convert:
...
class ExportCSVView(View):
    def get(self, request):
        ...
        show_fields = HideShowFilter.objects.filter(value=False, userID=get_user_id(request), parent=ModelChoices.ANALYSISOF_SECURITY_AUDITS_USER_LOGS)
        ...
        response['Content-Disposition'] = 'attachment; filename="analysis_of_security_audits_user_logs.csv"'       
        ...
        order_by = request.GET.get('order_by', 'ID')
        base_queryset = AnalysisofSecurityAuditsUserLogs.objects.filter(**filter_string).filter(combined_q_objects).filter(**snapshot_filter)
        queryset = base_queryset
        ...
        for software in softwares:
            row_data = []
            for field in fields:
                if field in AnalysisofSecurityAuditsUserLogs.date_fields_to_convert:
...
class ExportExcelView(View):
    ...
    def get(self, request):
        ...
        response['Content-Disposition'] = 'attachment; filename="analysis_of_security_audits_user_logs.xlsx"'
        wb.save(response)
        return response
...
class ExportPDFView(View):
    def get(self, request):
        ...
        response_pdf['Content-Disposition'] = 'attachment; filename="analysis_of_security_audits_user_logs.pdf"'
        ...
        softwares = AnalysisofSecurityAuditsUserLogs.objects.all()
