...
def create_privileged_users_vs_user_logs_filter(request):
    if request.method == 'POST':
        user_filter_common_func(request, 'PrivilegedusersVsUserLogs', 'PRIVILEGEDUSERS_VS_USER_LOGS')
    return redirect(request.META.get('HTTP_REFERER'))

def create_privileged_users_vs_user_logs_page_items(request):
    if request.method == 'POST':
        create_page_items(request, 'PRIVILEGEDUSERS_VS_USER_LOGS')
        return redirect(request.META.get('HTTP_REFERER'))

def create_privileged_users_vs_user_logs_hide_show_filter(request):
    if request.method == "POST":
        response_data = create_hide_show_filter(request, 'PRIVILEGEDUSERS_VS_USER_LOGS')
        return JsonResponse(response_data)

    return JsonResponse({'error': 'Invalid request'}, status=400)
    
def delete_privileged_users_vs_user_logs_filter(request, id):
    filter_instance = UserFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.PRIVILEGEDUSERS_VS_USER_LOGS)
    filter_instance.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_privileged_users_vs_user_logs_daterange_filter(request, id):
    date_filter = DateRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.PRIVILEGEDUSERS_VS_USER_LOGS)
    date_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_privileged_users_vs_user_logs_intrange_filter(request, id):
    int_filter = IntRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.PRIVILEGEDUSERS_VS_USER_LOGS)
    int_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_privileged_users_vs_user_logs_floatrange_filter(request, id):
    float_filter = FloatRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.PRIVILEGEDUSERS_VS_USER_LOGS)
    float_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def privileged_users_vs_user_logs(request):
    db_field_names = [field.name for field in PrivilegedusersVsUserLogs._meta.get_fields() if not field.is_relation]
    try:
        user_order = ColumnOrder.objects.get(
            user=request.user, 
            table_name='PrivilegedusersVsUserLogs',
    ...
    selected_rows = SelectedRows.objects.filter(
        model='privilegedusersvsuserlogs', 
        model_choice=ModelChoices.PRIVILEGEDUSERS_VS_USER_LOGS
    ).values_list('rows', flat=True)
    selected_rows = [int(item) for row in selected_rows for item in row.split(',')]

    # Snapshot
    latest_snapshot, summary, snapshot_filter, snapshots = common_snapshot_filter(request, 'privilegedusersvsuserlogs')

    field_names = []
    for field_name in ordered_fields:
        fields, created = HideShowFilter.objects.get_or_create(key=field_name, userID=get_user_id(request), parent=ModelChoices.PRIVILEGEDUSERS_VS_USER_LOGS)
        field_names.append(fields)

    page_items = PageItems.objects.filter(userID=get_user_id(request), parent=ModelChoices.PRIVILEGEDUSERS_VS_USER_LOGS).last()
    items = 25
    if page_items:
    ...
    filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.PRIVILEGEDUSERS_VS_USER_LOGS, field_type=FieldType.TEXT)
    ...
    # for date range
    date_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.PRIVILEGEDUSERS_VS_USER_LOGS, field_type=FieldType.DATE)
    ...
    # for integer range
    int_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.PRIVILEGEDUSERS_VS_USER_LOGS, field_type=FieldType.INT)
    ...
    # for float range
    float_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.PRIVILEGEDUSERS_VS_USER_LOGS, field_type=FieldType.FLOAT)
    ...
    base_queryset = PrivilegedusersVsUserLogs.objects.filter(**filter_string).filter(**pre_filter_string).filter(combined_q_objects).filter(**snapshot_filter)
    ...
    if user_unique_filter:
        queryset = common_unique_filter(request, user_unique_filter, queryset, snapshot_filter, 'tables_privilegedusersvsuserlogs')
    if date_unique_filter:
        queryset = common_unique_filter(request, date_unique_filter, queryset, snapshot_filter, 'tables_privilegedusersvsuserlogs')
    if int_unique_filter:
        queryset = common_unique_filter(request, int_unique_filter, queryset, snapshot_filter, 'tables_privilegedusersvsuserlogs')
    if float_unique_filter:
        queryset = common_unique_filter(request, float_unique_filter, queryset, snapshot_filter, 'tables_privilegedusersvsuserlogs')
    ...
    try:
        ...
        return redirect('privileged_users_vs_user_logs')
    except EmptyPage:
        return redirect('privileged_users_vs_user_logs') 

    read_only_fields = ('hash_data', 'ID', 'loader_instance', 'json_data',)
    pre_column = ('hash_data', 'ID', 'loader_instance', 'json_data',)
    compulsory_fields = ()
    
    COMBOS = {}
    #COMBOS['Contract_Type'] = TableDropdownSubItem.objects.filter(item__item='Contract_Type').values_list('subitem', flat=True)

    fields = get_model_fields('PrivilegedusersVsUserLogs', pre_column)
    saved_filters = list(SavedFilter.objects.filter(
        userID=request.user.id, parent=ModelChoices.PRIVILEGEDUSERS_VS_USER_LOGS
    ).values())
    ...
    context = {
        'segment'  : 'privileged_users_vs_user_logs',
        'parent'   : 'delta',
        'softwares' : softwares,
        ...
        'total_items': PrivilegedusersVsUserLogs.objects.filter(**pre_filter_string).filter(**snapshot_filter).count(),
        'pre_column': pre_column,
        'COMBOS': COMBOS,
        'date_picker_fields': PrivilegedusersVsUserLogs.date_fields_to_convert,
        'compulsory_fields': compulsory_fields,
        'fav_name': 'Privileged users Vs User Logs',
        'int_filter_instance': int_filter_instance,
        'int_fields': PrivilegedusersVsUserLogs.integer_fields,
        'float_filter_instance': float_filter_instance,
        'float_fields': PrivilegedusersVsUserLogs.float_fields,
        'encrypted_fields': PrivilegedusersVsUserLogs.encrypted_fields,
        ...
        'tabs': Tab.objects.filter(base_view='privileged_users_vs_user_logs').order_by('created_at'),
        'join_model_instance': PrivilegedusersVsUserLogs.join_model_instance if hasattr( PrivilegedusersVsUserLogs, 'join_model_instance') else None,
    ...
    return render(request, 'apps/privileged_users_vs_user_logs.html', context)


@login_required(login_url='/users/signin/')
def create_privileged_users_vs_user_logs(request):
    if request.method == 'POST':
        software_data = {}
        for attribute, value in request.POST.items():
            ...
            if attribute not in PrivilegedusersVsUserLogs.date_fields_to_convert:
                if attribute in PrivilegedusersVsUserLogs.integer_fields:
                    software_data[attribute] = int(value) if value else None
                elif attribute in PrivilegedusersVsUserLogs.float_fields:
        ...
        PrivilegedusersVsUserLogs.objects.create(**software_data)

    return redirect(request.META.get('HTTP_REFERER'))

@login_required(login_url='/users/signin/')
def delete_privileged_users_vs_user_logs(request, id):
    software = PrivilegedusersVsUserLogs.objects.get(ID=id)
    software.delete()
    return redirect(request.META.get('HTTP_REFERER'))


@login_required(login_url='/users/signin/')
def update_privileged_users_vs_user_logs(request, id):
    software = PrivilegedusersVsUserLogs.objects.get(ID=id)
    ...
    if request.method == 'POST':
        for attribute, value in request.POST.items():
            ...
            if not attribute in PrivilegedusersVsUserLogs.date_fields_to_convert:
...
class ExportCSVView(View):
    def get(self, request):
        ...
        show_fields = HideShowFilter.objects.filter(value=False, userID=get_user_id(request), parent=ModelChoices.PRIVILEGEDUSERS_VS_USER_LOGS)
        ...
        response['Content-Disposition'] = 'attachment; filename="privileged_users_vs_user_logs.csv"'       
        ...
        order_by = request.GET.get('order_by', 'ID')
        base_queryset = PrivilegedusersVsUserLogs.objects.filter(**filter_string).filter(combined_q_objects).filter(**snapshot_filter)
        queryset = base_queryset
        ...
        for software in softwares:
            row_data = []
            for field in fields:
                if field in PrivilegedusersVsUserLogs.date_fields_to_convert:
...
class ExportExcelView(View):
    ...
    def get(self, request):
        ...
        response['Content-Disposition'] = 'attachment; filename="privileged_users_vs_user_logs.xlsx"'
        wb.save(response)
        return response
...
class ExportPDFView(View):
    def get(self, request):
        ...
        response_pdf['Content-Disposition'] = 'attachment; filename="privileged_users_vs_user_logs.pdf"'
        ...
        softwares = PrivilegedusersVsUserLogs.objects.all()
