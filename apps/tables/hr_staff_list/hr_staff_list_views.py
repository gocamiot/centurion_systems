...
def create_hr_staff_list_filter(request):
    if request.method == 'POST':
        user_filter_common_func(request, 'HRStaffList', 'HR_STAFF_LIST')
    return redirect(request.META.get('HTTP_REFERER'))

def create_hr_staff_list_page_items(request):
    if request.method == 'POST':
        create_page_items(request, 'HR_STAFF_LIST')
        return redirect(request.META.get('HTTP_REFERER'))

def create_hr_staff_list_hide_show_filter(request):
    if request.method == "POST":
        response_data = create_hide_show_filter(request, 'HR_STAFF_LIST')
        return JsonResponse(response_data)

    return JsonResponse({'error': 'Invalid request'}, status=400)
    
def delete_hr_staff_list_filter(request, id):
    filter_instance = UserFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.HR_STAFF_LIST)
    filter_instance.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_hr_staff_list_daterange_filter(request, id):
    date_filter = DateRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.HR_STAFF_LIST)
    date_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_hr_staff_list_intrange_filter(request, id):
    int_filter = IntRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.HR_STAFF_LIST)
    int_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_hr_staff_list_floatrange_filter(request, id):
    float_filter = FloatRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.HR_STAFF_LIST)
    float_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def hr_staff_list(request):
    db_field_names = [field.name for field in HRStaffList._meta.get_fields() if not field.is_relation]
    try:
        user_order = ColumnOrder.objects.get(
            user=request.user, 
            table_name='HRStaffList',
    ...
    selected_rows = SelectedRows.objects.filter(
        model='hrstafflist', 
        model_choice=ModelChoices.HR_STAFF_LIST
    ).values_list('rows', flat=True)
    selected_rows = [int(item) for row in selected_rows for item in row.split(',')]

    # Snapshot
    latest_snapshot, summary, snapshot_filter, snapshots = common_snapshot_filter(request, 'hrstafflist')

    field_names = []
    for field_name in ordered_fields:
        fields, created = HideShowFilter.objects.get_or_create(key=field_name, userID=get_user_id(request), parent=ModelChoices.HR_STAFF_LIST)
        field_names.append(fields)

    page_items = PageItems.objects.filter(userID=get_user_id(request), parent=ModelChoices.HR_STAFF_LIST).last()
    items = 25
    if page_items:
    ...
    filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.HR_STAFF_LIST, field_type=FieldType.TEXT)
    ...
    # for date range
    date_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.HR_STAFF_LIST, field_type=FieldType.DATE)
    ...
    # for integer range
    int_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.HR_STAFF_LIST, field_type=FieldType.INT)
    ...
    # for float range
    float_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.HR_STAFF_LIST, field_type=FieldType.FLOAT)
    ...
    base_queryset = HRStaffList.objects.filter(**filter_string).filter(**pre_filter_string).filter(combined_q_objects).filter(**snapshot_filter)
    ...
    if user_unique_filter:
        queryset = common_unique_filter(request, user_unique_filter, queryset, snapshot_filter, 'tables_hrstafflist')
    if date_unique_filter:
        queryset = common_unique_filter(request, date_unique_filter, queryset, snapshot_filter, 'tables_hrstafflist')
    if int_unique_filter:
        queryset = common_unique_filter(request, int_unique_filter, queryset, snapshot_filter, 'tables_hrstafflist')
    if float_unique_filter:
        queryset = common_unique_filter(request, float_unique_filter, queryset, snapshot_filter, 'tables_hrstafflist')
    ...
    read_only_fields = ('ID', 'loader_instance', 'json_data', 'hash_data',)
    pre_column = ('ID', 'loader_instance', 'json_data', 'hash_data',)
    compulsory_fields = ()
    
    COMBOS = {}
    #COMBOS['Contract_Type'] = TableDropdownSubItem.objects.filter(item__item='Contract_Type').values_list('subitem', flat=True)

    fields = get_model_fields('HRStaffList', pre_column)
    saved_filters = list(SavedFilter.objects.filter(
        userID=request.user.id, parent=ModelChoices.HR_STAFF_LIST
    ).values())
    ...
    context = {
        ...
        'total_items': HRStaffList.objects.filter(**pre_filter_string).filter(**snapshot_filter).count(),
        'pre_column': pre_column,
        'COMBOS': COMBOS,
        'date_picker_fields': HRStaffList.date_fields_to_convert,
        'compulsory_fields': compulsory_fields,
        'fav_name': 'HR Staff List',
        'int_filter_instance': int_filter_instance,
        'int_fields': HRStaffList.integer_fields,
        'float_filter_instance': float_filter_instance,
        'float_fields': HRStaffList.float_fields,
        'encrypted_fields': HRStaffList.encrypted_fields,
        ...
        'tabs': Tab.objects.filter(base_view='hr_staff_list').order_by('created_at'),
        'join_model_instance': HRStaffList.join_model_instance if hasattr( HRStaffList, 'join_model_instance') else None,
    ...
    return render(request, 'apps/hr_staff_list.html', context)


@login_required(login_url='/users/signin/')
def create_hr_staff_list(request):
    if request.method == 'POST':
        software_data = {}
        for attribute, value in request.POST.items():
            ...
            if attribute not in HRStaffList.date_fields_to_convert:
                if attribute in HRStaffList.integer_fields:
                    software_data[attribute] = int(value) if value else None
                elif attribute in HRStaffList.float_fields:
        ...
        HRStaffList.objects.create(**software_data)

    return redirect(request.META.get('HTTP_REFERER'))

@login_required(login_url='/users/signin/')
def delete_hr_staff_list(request, id):
    software = HRStaffList.objects.get(ID=id)
    software.delete()
    return redirect(request.META.get('HTTP_REFERER'))


@login_required(login_url='/users/signin/')
def update_hr_staff_list(request, id):
    software = HRStaffList.objects.get(ID=id)
    ...
    if request.method == 'POST':
        for attribute, value in request.POST.items():
            ...
            if not attribute in HRStaffList.date_fields_to_convert:
...
class ExportCSVView(View):
    def get(self, request):
        ...
        show_fields = HideShowFilter.objects.filter(value=False, userID=get_user_id(request), parent=ModelChoices.HR_STAFF_LIST)
        ...
        response['Content-Disposition'] = 'attachment; filename="hr_staff_list.csv"'       
        ...
        order_by = request.GET.get('order_by', 'ID')
        base_queryset = HRStaffList.objects.filter(**filter_string).filter(combined_q_objects).filter(**snapshot_filter)
        queryset = base_queryset
        ...
        for software in softwares:
            row_data = []
            for field in fields:
                if field in HRStaffList.date_fields_to_convert:
...
class ExportExcelView(View):
    ...
    def get(self, request):
        ...
        response['Content-Disposition'] = 'attachment; filename="hr_staff_list.xlsx"'
        wb.save(response)
        return response
...
class ExportPDFView(View):
    def get(self, request):
        ...
        response_pdf['Content-Disposition'] = 'attachment; filename="hr_staff_list.pdf"'
        ...
        softwares = HRStaffList.objects.all()
