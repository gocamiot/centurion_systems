...
def create_user_logs_vs_privileged_users_filter(request):
    if request.method == 'POST':
        user_filter_common_func(request, 'UserLogsVsPrivilegedusers', 'USER_LOGS_VS_PRIVILEGEDUSERS')
    return redirect(request.META.get('HTTP_REFERER'))

def create_user_logs_vs_privileged_users_page_items(request):
    if request.method == 'POST':
        create_page_items(request, 'USER_LOGS_VS_PRIVILEGEDUSERS')
        return redirect(request.META.get('HTTP_REFERER'))

def create_user_logs_vs_privileged_users_hide_show_filter(request):
    if request.method == "POST":
        response_data = create_hide_show_filter(request, 'USER_LOGS_VS_PRIVILEGEDUSERS')
        return JsonResponse(response_data)

    return JsonResponse({'error': 'Invalid request'}, status=400)
    
def delete_user_logs_vs_privileged_users_filter(request, id):
    filter_instance = UserFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.USER_LOGS_VS_PRIVILEGEDUSERS)
    filter_instance.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_user_logs_vs_privileged_users_daterange_filter(request, id):
    date_filter = DateRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.USER_LOGS_VS_PRIVILEGEDUSERS)
    date_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_user_logs_vs_privileged_users_intrange_filter(request, id):
    int_filter = IntRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.USER_LOGS_VS_PRIVILEGEDUSERS)
    int_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def delete_user_logs_vs_privileged_users_floatrange_filter(request, id):
    float_filter = FloatRangeFilter.objects.get(id=id, userID=get_user_id(request), parent=ModelChoices.USER_LOGS_VS_PRIVILEGEDUSERS)
    float_filter.delete()
    return redirect(request.META.get('HTTP_REFERER'))

def user_logs_vs_privileged_users(request):
    db_field_names = [field.name for field in UserLogsVsPrivilegedusers._meta.get_fields() if not field.is_relation]
    try:
        user_order = ColumnOrder.objects.get(
            user=request.user, 
            table_name='UserLogsVsPrivilegedusers',
    ...
    selected_rows = SelectedRows.objects.filter(
        model='userlogsvsprivilegedusers', 
        model_choice=ModelChoices.USER_LOGS_VS_PRIVILEGEDUSERS
    ).values_list('rows', flat=True)
    selected_rows = [int(item) for row in selected_rows for item in row.split(',')]

    # Snapshot
    latest_snapshot, summary, snapshot_filter, snapshots = common_snapshot_filter(request, 'userlogsvsprivilegedusers')

    field_names = []
    for field_name in ordered_fields:
        fields, created = HideShowFilter.objects.get_or_create(key=field_name, userID=get_user_id(request), parent=ModelChoices.USER_LOGS_VS_PRIVILEGEDUSERS)
        field_names.append(fields)

    page_items = PageItems.objects.filter(userID=get_user_id(request), parent=ModelChoices.USER_LOGS_VS_PRIVILEGEDUSERS).last()
    items = 25
    if page_items:
    ...
    filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.USER_LOGS_VS_PRIVILEGEDUSERS, field_type=FieldType.TEXT)
    ...
    # for date range
    date_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.USER_LOGS_VS_PRIVILEGEDUSERS, field_type=FieldType.DATE)
    ...
    # for integer range
    int_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.USER_LOGS_VS_PRIVILEGEDUSERS, field_type=FieldType.INT)
    ...
    # for float range
    float_filter_instance = SavedFilter.objects.filter(userID=get_user_id(request), parent=ModelChoices.USER_LOGS_VS_PRIVILEGEDUSERS, field_type=FieldType.FLOAT)
    ...
    base_queryset = UserLogsVsPrivilegedusers.objects.filter(**filter_string).filter(**pre_filter_string).filter(combined_q_objects).filter(**snapshot_filter)
    ...
    if user_unique_filter:
        queryset = common_unique_filter(request, user_unique_filter, queryset, snapshot_filter, 'tables_userlogsvsprivilegedusers')
    if date_unique_filter:
        queryset = common_unique_filter(request, date_unique_filter, queryset, snapshot_filter, 'tables_userlogsvsprivilegedusers')
    if int_unique_filter:
        queryset = common_unique_filter(request, int_unique_filter, queryset, snapshot_filter, 'tables_userlogsvsprivilegedusers')
    if float_unique_filter:
        queryset = common_unique_filter(request, float_unique_filter, queryset, snapshot_filter, 'tables_userlogsvsprivilegedusers')
    ...
    try:
        ...
        return redirect('user_logs_vs_privileged_users')
    except EmptyPage:
        return redirect('user_logs_vs_privileged_users') 

    read_only_fields = ('json_data', 'hash_data', 'loader_instance', 'ID',)
    pre_column = ('json_data', 'hash_data', 'loader_instance', 'ID',)
    compulsory_fields = ()
    
    COMBOS = {}
    #COMBOS['Contract_Type'] = TableDropdownSubItem.objects.filter(item__item='Contract_Type').values_list('subitem', flat=True)

    fields = get_model_fields('UserLogsVsPrivilegedusers', pre_column)
    saved_filters = list(SavedFilter.objects.filter(
        userID=request.user.id, parent=ModelChoices.USER_LOGS_VS_PRIVILEGEDUSERS
    ).values())
    ...
    context = {
        'segment'  : 'user_logs_vs_privileged_users',
        'parent'   : 'delta',
        'softwares' : softwares,
        ...
        'total_items': UserLogsVsPrivilegedusers.objects.filter(**pre_filter_string).filter(**snapshot_filter).count(),
        'pre_column': pre_column,
        'COMBOS': COMBOS,
        'date_picker_fields': UserLogsVsPrivilegedusers.date_fields_to_convert,
        'compulsory_fields': compulsory_fields,
        'fav_name': 'User Logs Vs Privileged users',
        'int_filter_instance': int_filter_instance,
        'int_fields': UserLogsVsPrivilegedusers.integer_fields,
        'float_filter_instance': float_filter_instance,
        'float_fields': UserLogsVsPrivilegedusers.float_fields,
        'encrypted_fields': UserLogsVsPrivilegedusers.encrypted_fields,
        ...
        'tabs': Tab.objects.filter(base_view='user_logs_vs_privileged_users').order_by('created_at'),
        'join_model_instance': UserLogsVsPrivilegedusers.join_model_instance if hasattr( UserLogsVsPrivilegedusers, 'join_model_instance') else None,
    ...
    return render(request, 'apps/user_logs_vs_privileged_users.html', context)


@login_required(login_url='/users/signin/')
def create_user_logs_vs_privileged_users(request):
    if request.method == 'POST':
        software_data = {}
        for attribute, value in request.POST.items():
            ...
            if attribute not in UserLogsVsPrivilegedusers.date_fields_to_convert:
                if attribute in UserLogsVsPrivilegedusers.integer_fields:
                    software_data[attribute] = int(value) if value else None
                elif attribute in UserLogsVsPrivilegedusers.float_fields:
        ...
        UserLogsVsPrivilegedusers.objects.create(**software_data)

    return redirect(request.META.get('HTTP_REFERER'))

@login_required(login_url='/users/signin/')
def delete_user_logs_vs_privileged_users(request, id):
    software = UserLogsVsPrivilegedusers.objects.get(ID=id)
    software.delete()
    return redirect(request.META.get('HTTP_REFERER'))


@login_required(login_url='/users/signin/')
def update_user_logs_vs_privileged_users(request, id):
    software = UserLogsVsPrivilegedusers.objects.get(ID=id)
    ...
    if request.method == 'POST':
        for attribute, value in request.POST.items():
            ...
            if not attribute in UserLogsVsPrivilegedusers.date_fields_to_convert:
...
class ExportCSVView(View):
    def get(self, request):
        ...
        show_fields = HideShowFilter.objects.filter(value=False, userID=get_user_id(request), parent=ModelChoices.USER_LOGS_VS_PRIVILEGEDUSERS)
        ...
        response['Content-Disposition'] = 'attachment; filename="user_logs_vs_privileged_users.csv"'       
        ...
        order_by = request.GET.get('order_by', 'ID')
        base_queryset = UserLogsVsPrivilegedusers.objects.filter(**filter_string).filter(combined_q_objects).filter(**snapshot_filter)
        queryset = base_queryset
        ...
        for software in softwares:
            row_data = []
            for field in fields:
                if field in UserLogsVsPrivilegedusers.date_fields_to_convert:
...
class ExportExcelView(View):
    ...
    def get(self, request):
        ...
        response['Content-Disposition'] = 'attachment; filename="user_logs_vs_privileged_users.xlsx"'
        wb.save(response)
        return response
...
class ExportPDFView(View):
    def get(self, request):
        ...
        response_pdf['Content-Disposition'] = 'attachment; filename="user_logs_vs_privileged_users.pdf"'
        ...
        softwares = UserLogsVsPrivilegedusers.objects.all()
